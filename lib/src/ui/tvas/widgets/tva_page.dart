...
class _TvaPageState extends State<TvaPage> {
  ...
  List<PieChartSectionData> _generatePieSections(Tva tvaData) {
    ...
    return tvaData.items.asMap().entries.map((entry) {
      ...
      return PieChartSectionData(
        color: sectionColor,
        value: itemTvaValue, // The actual value for the section
        title: '${item.code}% (${percentage.toStringAsFixed(1)}%)', // Display TVA code and its percentage
        radius: radius,
        titleStyle: TextStyle(
          fontSize: fontSize,
          fontWeight: FontWeight.bold,
          color: ThemeData.estimateBrightnessForColor(sectionColor) == Brightness.dark
              ? Colors.white : Colors.black54, // Adjust text color for contrast
            ...
            child: Consumer<TvaService>(
              builder: (context, tvaService, child) {
                return ServiceStateWrapper<TvaService, Tva>(
                  service: tvaService,
                  data: tvaService.tva,
                  customEmptyDataMessage: 'Aucune donnée pour la période sélectionnée.',
                  successBuilder: (BuildContext context, Tva tvaData) {
                    final pieSections = _generatePieSections(tvaData);

                    return RefreshIndicator(
                      onRefresh: _fetchData,
                      child: LayoutBuilder( // Use LayoutBuilder to get constraints for responsive sizing
                        builder: (context, constraints) {
                          // Determine if we should show chart and table side-by-side or stacked
                          bool isWideScreen = constraints.maxWidth > 600; // Example breakpoint
                          ...
                          if (isWideScreen) {
                            ...
                            return Row(
                              ...
                              children: [
                                ...
                                Expanded(
                                  ...
                                  child: _buildDataTable(tvaData, constraints,textTheme),
                                ),

                                const VerticalDivider(width: 1, thickness: 1),
                                Expanded(
                                  flex: 3, // Give more space to the chart or adjust as needed
                                  child: _buildPieChartCard(pieSections, textTheme),
                                ),
                              ],
                            );
                          } else {
                            ...
                            return SingleChildScrollView(
                              ...
                              child: Column(
                                children: [
                                  _buildDataTable(tvaData, constraints,textTheme),
                                  const SizedBox(height: 8.0), // Spacing between table and chart
  ...
  Widget _buildDataTable(Tva tvaData, BoxConstraints constraints, TextTheme textTheme) { // Added textTheme for potential title
    return Card(
      elevation: 0.2, // You can adjust elevation
      margin: const EdgeInsets.all(2),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      clipBehavior: Clip.antiAlias,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch, // Make column children take full width
        children: [
          Padding( // Padding for the title
            padding: const EdgeInsets.only(top: 4.0, left: 4.0, right: 4.0, bottom: 8.0),
            child: Text(
          ...
          SingleChildScrollView(
            ...
            child: ConstrainedBox(
              ...
              constraints: BoxConstraints(
                  minWidth: constraints.maxWidth < 600 ? constraints.maxWidth : constraints.maxWidth / 2
              ),
              child: DataTable(
                ...
                columns: const <DataColumn>[
                  DataColumn(
                    label:  SizedBox(
    width: 35,
    child: Text('Code',textAlign: TextAlign.left, style: TextStyle(fontWeight: FontWeight.bold)),
    ),
                  ),
                  DataColumn(
                    label: Text('TTC', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right),
                    numeric: true,
                  ),
                  DataColumn(
                    label: Text('TVA', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right),
                    numeric: true,
                  ),
                  DataColumn(
                    label: Text('HT', style: TextStyle(fontWeight: FontWeight.bold), textAlign: TextAlign.right),
                    numeric: true,
                  ),
                ],
                rows: tvaData.items.map((item) {
                  ...
                  child: Text(   textAlign: TextAlign.left,item.code.toString()),

                  )),

                      DataCell(Align(alignment: Alignment.centerRight, child: Text(item.ttc))),
                      DataCell(Align(alignment: Alignment.centerRight, child: Text(item.tva))),
                      DataCell(Align(alignment: Alignment.centerRight, child: Text(item.ht))),
                    ],
                  );
                }).toList(),
              ),
  ...
  Widget _buildLegend(List<PieChartSectionData> sections, TextTheme textTheme) {
    return Wrap( // Use Wrap for a responsive legend
      spacing: 16.0,
      runSpacing: 8.0,
  ...
  Widget _buildPieChartCard(List<PieChartSectionData> pieSections, TextTheme textTheme) {
    return Card(
      elevation: 0.2,
      margin: const EdgeInsets.all(2),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      ...
      color: Theme.of(context).dividerColor.withValues(alpha: 0.5),
              ...
              AspectRatio(
                ...
                child: PieChart( // The PieChart widget itself
                  PieChartData(  // All configuration goes into PieChartData
                    ...
                    centerSpaceRadius: 50, // Radius of the center hole (0 for a full pie)
